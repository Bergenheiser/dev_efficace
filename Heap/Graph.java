    ...
    public ArrayList<Integer> dijkstraWithoutPriorityQ(int s, int t) {
        ...
        while (!trouve && avoir.size() > 0) {
            ...
            for (int i = 1; i < avoir.size(); i++) {
                if (dist[avoir.get(i)] < dist[avoir.get(indiceMin)])
                    indiceMin = i;
            }
            int u = avoir.remove(indiceMin);
            if (u == t) {
                trouve = true;
            } else {
                ArrayList<Couple<Integer, Double>> listVois = l.get(u);
                for (Couple<Integer, Double> vois : listVois) {
                    int v = vois.getElement1();
                    double c = vois.getElement2(); //cout de l'arrête uv
                    if (dist[u] + c < dist[v]) {
                        pred[v] = u;
                        //mise à jour nécessaire pour v
